...
static int app_verif_exec(struct ast_channel *chan, ast_app_data data)
{
	...
	/* Start recognition. */
	if (verif_channel_start(app_session->recog_channel, name, start_input_timers, mrcprecog_options.recog_hfs) != 0) {
...
int load_mrcpverif_app()
{
	...
	if ((mrcpverif->app = mrcp_application_create(recog_message_handler, (void *)0, pool)) == NULL) {
	...
	mrcpverif->audio_stream_vtable.open_rx = recog_stream_open;
	mrcpverif->audio_stream_vtable.close_rx = NULL;
	mrcpverif->audio_stream_vtable.read_frame = recog_stream_read;
